...
> **Status:** the canonical profile in [SPEC-001](docs/specs/001-canonical-ledger.md) is verified. Lossless loading, vault validation, conflict-safe mutation, Evidence capture, bounded filesystem search/inspect, deterministic exact-commit JSONL export, the read/query-only CLI through BK-012, and Pi read/search/validate/write/checkpoint workflows through BK-015 are merged and verified alongside dependency/CI maintenance through BK-024. ADR-0008 defers all filesystem-writing CLI commands without weakening publication guarantees. The authenticated retrieval-service protocol and threat-reviewed Redis projection design in BK-016 are merged and verified; BK-017 authenticated lexical indexing/search is the next Ready item. See the ordered [backlog](docs/planning/backlog.md).
...
packages/core/         lossless parsing, validation, safe mutation, and local query
