...
5. Bound source traversal, concepts, aggregate concept/resource bytes, diagnostics, encoded output, and cancellation; invalid or incomplete snapshots produce zero JSONL bytes.
6. Separate pure mapping/planning from network execution.
7. Produce a dry-run plan containing create/update/skip/conflict actions and field-loss warnings.
8. Use Bookie UID plus recorded external ID to make reruns idempotent.
9. Never infer destructive deletes by default.
10. Write external IDs back through the canonical mutation path only after confirmed success and explicit approval.
11. Preserve an execution receipt linking destination results, canonical source commit, mapper version, and plan hash.
12. Implement Jira CSV/API, Asana CSV, and Trello API/JSON as separate adapters only after fixture mappings are approved.
13. Bound and hash attachments; disclose destination size/type limitations.
...
- Invalid/incomplete commits, output bounds, and unclassified records invoke the sink zero times; sink failure and cancellation disclose or document discard of a possible prefix.
