...
**Controls:** environment/secret-manager credentials; excluded path and sensitivity policies; default pre-export, pre-write, and pre-index secret scanning; metadata-only query logs; hierarchical redaction of excluded concept, OKF source-resource, project, relation, support, Evidence-resource, and local-link paths; static error messages; no credential fields in canonical schemas. Canonical export's deterministic local scan is enabled by default; its low-level `allow-unchecked` opt-out is explicit, reported in the result, unavailable through environment configuration, and never bypasses sensitivity exclusions. Per ADR-0007, supported write surfaces scan complete resulting concepts and bounded staged Evidence resources before publication, return only static redacted failures, and expose no unchecked option. ADR-0008 exposes no release 0.1 CLI write surface. Any later user-facing override requires a superseding approval and audit design.
