...
| REQ-026 | User-facing Bookie operations shall not write detected secrets to canonical records or Evidence resources, and secrets shall not be indexed, checkpointed, or logged. Excluded sensitivity classes shall not be indexed, checkpointed, logged, or exported. Export secret detection shall default on; only an explicit low-level unchecked policy may bypass export detection, never sensitivity exclusions. |
